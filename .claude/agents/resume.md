...
6. Write tailored resume to `search/vault/generated/resumes/{company-slug}-{role-slug}-v{N}.json`.
7. Link the resume to the open role — call the update-status API:
   ```bash
   curl -s -X POST http://localhost:8791/api/finding/open-roles/update-status \
     -H 'Content-Type: application/json' \
     -d '{"id":"{role_id if known}","company":"{company}","title":"{role}","status":"resume-ready","resume_file":"vault/generated/resumes/{slug}-v{N}.json"}'
   ```
   If a Role ID was provided in the directive (look for "Role ID: ..."), include it as `id`. Otherwise omit — the API matches by company+title.
8. Post completion summary to blackboard:
   ```
   write_to_blackboard path="agents.resume"
...
### 3. Output
- Write tailored resume to `search/vault/generated/resumes/{company-slug}-resume.md`
...
- Write resume output to `search/vault/generated/resumes/`
...
|---------|-------------|---------------|
| Resume tailored and saved | coach | "Resume ready for {company} {role} at {file path}" |
| Cover letter written | NONE | Just update your status. |
