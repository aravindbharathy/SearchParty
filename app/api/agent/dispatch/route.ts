...
// Map directive keywords to skill files so dispatched agents follow structured flows
const SKILL_HINTS: Array<{ pattern: RegExp; skill: string }> = [
  { pattern: /target.compan|generate.*target|ranked.*list/i, skill: 'generate-targets' },
  { pattern: /company.research|research.*compan|build.*intel/i, skill: 'company-research' },
  { pattern: /score.*jd|job.description.*score/i, skill: 'score-jd' },
  { pattern: /resume.*tailor|tailor.*resume/i, skill: 'resume-tailor' },
  { pattern: /cover.letter/i, skill: 'cover-letter' },
  { pattern: /interview.*prep/i, skill: 'interview-prep' },
  { pattern: /mock.*interview/i, skill: 'mock-interview' },
  { pattern: /interview.*debrief/i, skill: 'interview-debrief' },
  { pattern: /connection.*request|outreach.*batch/i, skill: 'connection-request' },
  { pattern: /referral/i, skill: 'referral-request' },
  { pattern: /linkedin.*audit/i, skill: 'linkedin-audit' },
  { pattern: /salary.*research/i, skill: 'salary-research' },
  { pattern: /negoti/i, skill: 'negotiate' },
  { pattern: /scan.*role|open.*role/i, skill: 'scan-roles' },
]

function matchSkill(text: string): string | null {
  // Check for explicit "Run skill: X" pattern first (from coach directives)
  const explicit = text.match(/Run skill:\s*([a-z-]+)/i)
  if (explicit) return explicit[1]
  // Fall back to keyword matching
  for (const { pattern, skill } of SKILL_HINTS) {
