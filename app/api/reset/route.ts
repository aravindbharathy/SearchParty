...
export async function POST(req: Request) {
  try {
    ...
    const generatedSubs = ['resumes', 'cover-letters', 'outreach', 'prep', 'messages', 'closing', 'linkedin-audit']
    ...
    // Reset open roles (pre-create with empty structure so scan agents can append)
    writeFileSync(join(searchDir, 'pipeline', 'open-roles.yaml'), 'roles: []\nlast_scan: null\nscan_count: 0\n')
    // Reset messages
    writeFileSync(join(searchDir, 'pipeline', 'messages.yaml'), 'messages: []\n')
    ...
    if (full) {
      ...
      const emptySchemas: Record<string, object> = {
        ...
        'career-plan': {
          ...
          deal_breakers: [],
          visa_status: '',
          work_style: { environment: '' },
          role_preferences: { track: '' },
          what_matters: [],
      ...
      const stateFileDefaults: Record<string, string> = {
        'board.md': '# Daily Briefing\n\n_No briefing yet._\n',
        'snapshot.yaml': '',
        'playbook.yaml': YAML.stringify({ lessons: [], decisions: [], checklists: [] }),
      }
      for (const [f, content] of Object.entries(stateFileDefaults)) {
