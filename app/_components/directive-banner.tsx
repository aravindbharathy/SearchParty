...
function humanize(text: string, agent: string): string {
  ...
  // Specific skill matches first (before agent-level catch-alls)
  if (t.includes('ats-check') || t.includes('ats check') || t.includes('ats compatibility')) {
    return 'ATS compatibility check completed'
  }

  if (t.includes('recruiter-review') || t.includes('recruiter review')) {
    return 'Recruiter review completed'
  }

  if (t.includes('cover-letter') || t.includes('cover letter')) {
    const company = text.match(/for\s+(\S+(?:\s+\S+)?)/i)?.[1] || ''
    return company ? `Cover letter ready for ${company}` : 'A new cover letter is ready'
  }

  if (t.includes('tailor resume') || t.includes('tailored resume') || t.includes('resume-tailor')) {
    const company = text.match(/for\s+(\S+(?:\s+\S+)?)/i)?.[1] || ''
  ...
  if (t.includes('target compan') || t.includes('generate-targets') || t.includes('ranked') || t.includes('company list')) {
    return 'Target company list generated'
  }

  if (t.includes('company-research') || (t.includes('intel') && t.includes('build'))) {
    const company = text.match(/research\s+(?:the\s+)?(?:top\s+\d+\s+)?(\S+)/i)?.[1] || ''
    return company ? `Company intel built for ${company}` : 'Company research completed'
  }

  if (t.includes('scan-roles') || t.includes('scan.*role')) {
    return 'Open roles scanned'
  }
