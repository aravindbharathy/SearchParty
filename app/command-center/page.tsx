...
function agentBadgeColor(name: string | undefined | null): string {
  if (!name) return 'bg-text-muted/10 text-text-muted border-text-muted/20'
  return AGENT_COLORS[String(name).toLowerCase()] ?? 'bg-text-muted/10 text-text-muted border-text-muted/20'
}
    ...
    try {
      ...
      if (res.ok) {
        ...
        const keysToRemove = [
          ...
          'networking-chat-messages', 'networking-active-tab', 'networking-parsed-messages',
          ...
          'agent-welcome-seen-research', 'agent-welcome-seen-resume', 'agent-welcome-seen-networking',
          'agent-welcome-seen-interview', 'agent-welcome-seen-negotiation', 'agent-welcome-seen-coach',
          'dismissed-directive-notifications',
        ]
                  ...
                  {Object.entries(agents).map(([name, data]) => {
                    ...
                    return (
                      <div key={name} className="bg-bg border border-border/60 rounded-md p-3 text-xs">
                        ...
                        {typeof a.current_task === 'string' && a.current_task && <p className="text-text-muted mt-1"><span className="text-text font-medium">Working on:</span> {a.current_task}</p>}
                        {typeof a.last_task === 'string' && a.last_task && <p className="text-text-muted mt-1"><span className="text-text font-medium">Last task:</span> {a.last_task}</p>}
