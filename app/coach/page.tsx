...
const SECTION_KEYWORDS: Record<SectionKey, string[]> = {
  ...
  'interview-answers': ['section 3:', 'phase 3:', 'your story section', 'interview prep answers', 'salary expectation', 'why are you leaving', 'saving to interview-answers'],
...
Start with a warm greeting — introduce yourself as their job search coach. Check search/vault/uploads/resumes/ for resume files.
...
7. "What's your visa/work authorization status?" (US Citizen, Green Card, H1B, OPT, etc.)
...
- After getting role/level/industries/comp/remote/track/visa → Write to search/context/career-plan.yaml (target section + visa_status)
...
BEFORE asking the next question, WRITE to search/context/career-plan.yaml with the updated what_matters, deal_breakers fields.

### Phase 3: Your Story (5 min) — why you're searching and interview prep
13. "What's driving your search? What are you moving TOWARD in your next role?"
14. "Why are you leaving your current role?" — help them frame it positively
15. "What would you say is your greatest weakness?" — help craft a genuine answer

BEFORE asking the next question, WRITE to search/context/interview-answers.yaml with:
- why_searching: what they're moving toward
- why_leaving: their answer about leaving current role
- greatest_weakness: their prepared weakness answer
- addressing_weaknesses: strategies for gaps

Note: visa_status lives in career-plan.yaml (search constraint, asked in Phase 1). Salary expectations is optional here — comp floor is already in career-plan.

### Phase 4: Resume Deep-Dive (10 min) — now go deep
Check search/vault/uploads/resumes/ for uploaded files. If found, parse the most recent one.
...
- CRITICAL: You MUST always end your turn with a text response to the user. After writing files with the Write/Edit tools, you MUST still provide a text message confirming what was saved and asking the next question. If your last action is a tool call with no follow-up text, the user will see nothing. NEVER end on a tool call — always follow with text.`
...
CRITICAL: You MUST always end your turn with a text response to the user. After writing files with Write/Edit tools, you MUST still provide a text message. If your last action is a tool call with no follow-up text, the user will see nothing. NEVER end on a tool call — always follow with text.`
...
const SECTION_META: Record<string, { icon: string; description: string; label: string }> = {
  ...
  'career-plan': { icon: '\uD83C\uDFAF', description: 'Target roles, preferences, priorities', label: "What You're Looking For" },
  'interview-answers': { icon: '\uD83D\uDCAC', description: 'Why searching, interview prep, visa', label: 'Your Story' },
...
function ContextPreviewCompact({ name, data }: { name: string; data: Record<string, unknown> }) {
  switch (name) {
    ...
    case 'interview-answers': return <QAMasterPreview data={data} />
...
/** Generic field-by-field display from schema status data */
function SchemaFieldList({ section, data }: { section: { fields?: Record<string, { label: string; filled: boolean; value_summary: string }> }; data: Record<string, unknown> }) {
  const fields = section.fields || {}
  return (
    <div className="text-xs text-text-muted space-y-1">
      {Object.entries(fields).map(([path, f]) => {
        // Resolve nested value for display
        const val = path.split('.').reduce((o: unknown, k) => (o as Record<string, unknown>)?.[k], data as unknown)
        let display = ''
        if (Array.isArray(val)) {
          display = val.length > 0 ? val.slice(0, 3).map(v => typeof v === 'string' ? v : typeof v === 'object' && v !== null ? ((v as Record<string, string>).name || (v as Record<string, string>).role || (v as Record<string, string>).company || JSON.stringify(v).slice(0, 40)) : String(v)).join(', ') + (val.length > 3 ? ` +${val.length - 3} more` : '') : ''
        } else if (typeof val === 'number' && val > 0) {
          display = val.toLocaleString()
        } else if (typeof val === 'string') {
          display = val.length > 60 ? val.slice(0, 60) + '...' : val
        }
        return (
          <p key={path}>
            {f.label}: {display ? <span className="text-text">{display}</span> : <span className="italic">not set</span>}
          </p>
        )
      })}
    </div>
...
/** Expanded view — shows all fields from schema */
function ContextPreviewExpanded({ name, data, sectionStatus }: { name: string; data: Record<string, unknown>; sectionStatus?: { fields?: Record<string, { label: string; filled: boolean; value_summary: string }> } }) {
  // For sections with schema status data, use the generic field list
  if (sectionStatus?.fields && (name === 'career-plan' || name === 'interview-answers')) {
    return <SchemaFieldList section={sectionStatus} data={data} />
  }
...
function KeyInfoLine({ sectionKey, data, filled, section }: { sectionKey: string; data?: Record<string, unknown>; filled: boolean; section: ProfileSectionStatus }) {
  ...
  switch (sectionKey) {
    ...
    case 'interview-answers': {
      ...
      const filledCount = allFields.filter(([, f]) => (f as { filled?: boolean }).filled).length
      if (filledCount === 0) return <p className="text-sm text-text-muted italic">Not started</p>
      const missing = allFields.filter(([, f]) => { const ff = f as { required?: boolean; filled?: boolean; label: string }; return ff.required && !ff.filled }).map(([, f]) => (f as { label: string }).label)
      if (missing.length > 0) return <p className="text-sm text-text-muted">{filledCount} answered · <span className="text-danger">Missing: {missing.join(', ')}</span></p>
      return <p className="text-sm text-text-muted">{filledCount} answers set</p>
    }
    case 'target-companies': {
            ...
            >
              ...
              {isExpanded && (
                <div className="px-3 pb-3 border-t border-border/50 pt-2">
                  {contextData[key] && (
                    <div className="mb-2">
                      <ContextPreviewExpanded name={key} data={contextData[key]} sectionStatus={sect} />
                    </div>
                  )}
...
export default function CoachPage() {
  ...
  const fetchContextStatus = useCallback(async () => {
    try {
      ...
      if (res.ok) {
        ...
        // Check if any file changed since last poll
        const modTimes = Object.values(data.sections || {} as Record<string, { lastModified?: string }>).map((s) => (s as { lastModified?: string }).lastModified || '').join(',')
        if (modTimes !== lastModifiedRef.current) {
          lastModifiedRef.current = modTimes
          return true // signal: data changed
        }
      }
    } catch {}
    return false
  ...
  const fetchAllContextData = useCallback(async () => {
    const names = ['experience-library', 'career-plan', 'interview-answers', 'target-companies', 'connection-tracker']
    try {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      const changed = await fetchContextStatus()
      if (changed) fetchAllContextData()
    }, 5000)
  ...
  const handleCoachOutput = useCallback((output: string) => {
    ...
    setMessages((prev) => [...prev, { role: 'coach', content: output }])
    const detected = detectSection(output)
    if (detected) {
  ...
  }, [agentStatus, agentOutput, agentReset, fetchContextStatus, fetchAllContextData, handleCoachOutput])
  ...
  const handleResumeUploaded = () => {
    sendMessage("I've uploaded my resume to vault/uploads/resumes/")
  }
    ...
    <div className="flex flex-col lg:flex-row h-full overflow-hidden">
      ...
      <div className="flex-1 lg:w-[60%] flex flex-col bg-white dark:bg-surface min-h-0">
        ...
        <div className="border-t border-border px-4 py-3 bg-surface">
          ...
          <p className="text-xs text-text-muted mb-1.5 flex items-center gap-1">
            <span>{'\uD83C\uDF99\uFE0F'}</span>
            <span>Tip: Try dictating your answers instead of typing — speaking often produces richer, more natural responses.</span>
          </p>
      ...
      {editingSection && editData && (
        <ModalOverlay onClose={() => { setEditingSection(null); setEditData(null) }}>
          ...
          {editingSection === 'interview-answers' && (
