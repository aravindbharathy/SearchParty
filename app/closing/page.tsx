...
const NEGOTIATION_DIRECTIVE = `You are the user's negotiation specialist. Read search/context/career-plan.yaml, search/pipeline/offers.yaml, search/pipeline/open-roles.yaml, and any salary research files in search/vault/generated/closing/ for context. Open roles shows which roles have been scored, have resumes, and their application status — use this to understand the full picture when negotiating.
...
export default function ClosingPage() {
  const [activeTab, setActiveTab] = useState<TabKey>(() => {
    ...
    try { const s = localStorage.getItem('closing-active-tab') as TabKey; if (s) return s } catch {}
    return 'offers'
  })
  ...
  const [pipelineApps, setPipelineApps] = useState<Array<{ company: string; role: string; status: string }>>([])
  ...
  useAgentWelcome('negotiation', 'I\'m your negotiation specialist. I can help with salary research, offer analysis, and building negotiation strategies.\n\nWhat would you like to work on?', chatMessages, setChatMessages, 'closing-chat-messages')
  ...
  useEffect(() => {
    loadOffers()
    loadDocs()
    // Load pipeline apps for the offer form picker
    fetch('/api/pipeline/applications').then(r => r.ok ? r.json() : { applications: [] })
      .then(data => setPipelineApps((data.applications || []).filter((a: { status: string }) => a.status !== 'rejected' && a.status !== 'withdrawn')))
      .catch(() => {})
            ...
            <div>
              ...
              {salaryDocs.length === 0 ? (
                ...
                <div className="space-y-2">
                  {salaryDocs.map(doc => (
                    <div key={doc.filename} className="p-4 border border-border rounded-lg">
