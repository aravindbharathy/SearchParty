...
export function loadProfileSchema(): ProfileSchema {
  ...
  // Check lib/ first (tracked in git), fall back to search/config/ (legacy)
  const trackedPath = join(process.cwd(), 'lib', 'profile-schema.yaml')
  const legacyPath = join(getSearchDir(), 'config', 'profile-schema.yaml')
  const schemaPath = existsSync(trackedPath) ? trackedPath : legacyPath
  if (!existsSync(schemaPath)) {
    throw new Error(`Profile schema not found at ${trackedPath} or ${legacyPath}`)
  }
