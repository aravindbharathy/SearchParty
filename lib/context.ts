...
const ExperienceEntrySchema = z.object({
  ...
  if (!e.dates && e.start_date) {
    e.dates = `${e.start_date}${e.end_date ? ` — ${e.end_date}` : ' — Present'}`
  }
...
const EducationSchema = z.object({
  ...
  if (!e.year && e.graduation_date) e.year = String(e.graduation_date)
  if (!e.year && e.graduation_year) e.year = String(e.graduation_year)
  return e as typeof entry
})
...
const InterviewEntrySchema = z.object({
  company: z.string().default(''),
  role: z.string().default(''),
...
export const InterviewHistorySchema = z.object({
  interviews: z.array(InterviewEntrySchema).default([]),
...
export const CONTEXT_FILES = {
  ...
  'interview-answers': {
    ...
    filename: 'interview-answers.yaml',
    label: 'Your Story',
    description: 'Why searching, interview prep, weaknesses',
  },
  'target-companies': {
