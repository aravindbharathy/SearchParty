...
search/                         <- Operations state directory
  ...
  pipeline/                     <- Role lifecycle and application tracking
    open-roles.yaml             <- Canonical role record (id, score, score_file, resume_file, application_ids)
    applications.yaml           <- Application submissions (role_id links back to open-roles)
    interviews.yaml             <- Interview records
    offers.yaml                 <- Offer records
